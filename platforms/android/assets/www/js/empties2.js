...
//form += ' <label>Outlet</label>';
//form += ' <select id="Monday" name="outlets[]" class="form-control multi_select" multiple>';
//form += ' </select>';
//form += ' </div>';
form += ' <div class="form-group list-group-item">';
form += ' <label>Empties in case</label>';
form += ' <input type="text" id="empties_case" class="form-control" placeholder="Empties in Case"/>';
form += ' </div>';
form += ' <div class="form-group list-group-item">';
form += ' <label>Empty Shells</label>';
form += ' <input type="text" id="empties_shell" class="form-control" placeholder="Empties Shells"/>';
form += ' </div>';
form += ' <div class="form-group list-group-item">';
form += ' <label>Comments</label>';
form += ' <textarea id="comments" class="form-control" cols="30" rows="5"></textarea>';
form += ' </div>';
...
$(document).ready(function() {
    ...
    $('#form-modal').on('show.bs.modal', function (event) {
        ...
        $('#displaySubmit').on('click', function () {
            console.log('trying to save outlet empties');
    ...
    if (dbid == 0){
        ...
        db.transaction(function (t) {
            t.executeSql(q, null, function (t, data) {
                ...
                table += ' <label>Editing Outlet Empties:</label>';
                table += ' </div>';
...
function fetchEmpties(){
    var week = moment().week();
    var month = thisMonth();
    var year = thisYear();
    var q = "SELECT * FROM empties WHERE week = '"+week+"' AND month = '"+month+"' AND year = '"+year+"'";
...
function insertPlan(){
    ...
    db.transaction(function(st) {
        ...
        st.executeSql('CREATE TABLE IF NOT EXISTS empties (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, coords VARCHAR, routeplan LONGTEXT, submitter VARCHAR, inputdate VARCHAR, week VARCHAR, month VARCHAR, year VARCHAR, modified TEXT, created TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,last_sync TEXT DEFAULT "none")');
        st.executeSql("INSERT INTO empties(coords,routeplan,submitter,inputdate,week,month,year) values (?,?,?,?,?,?,?)",[coords,routedata,submitter,inputdate,week,month,year], alertSuccess);
...
function syncTasks(){
  ...
  var q = "SELECT * FROM empties WHERE last_sync = 'none'";
  ...
  db.transaction(function (t) {
    t.executeSql(q, null, function (t, data) {
      ...
      if (found >= 1) {
         ...
         msg += '<p>You do not have any unsynced outlet empties</p>';
      }
...
function ajaxUpload(jsondata){
   $.ajax({
        url : ServerURI+'/api/postdata.php?info=empties&username='+username,
        type : 'POST',
        data : jsondata,
        ...
        error : function(xhr, status, error){
            ...
            $('.syncmessage').html('There was an error uploading outlet empties');
        },
  ...
  if (itemids.length){
      db.transaction(function(st) {
          for (var i = itemids.length - 1; i >= 0; i--) {
            ...
            st.executeSql("UPDATE empties SET last_sync = '"+lastsync+"' WHERE id = '"+itemids[i]+"'",null, alertSuccess);
          };
      },onError,function(){
