...
function saveItem() {
  var available_wl_1l = document.getElementById("available_wl_1l").value;
  var available_wl_35cl = document.getElementById("available_wl_35cl").value;
  var available_wl_75cl = document.getElementById("available_wl_75cl").value;
  ...
  db.transaction(function(st) {
    ...
    st.executeSql('CREATE TABLE IF NOT EXISTS availability (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,coords VARCHAR, available_wl_1l VARCHAR,available_wl_35cl VARCHAR,available_wl_75cl VARCHAR,submitter VARCHAR,store VARCHAR,store_id INTEGER, store_server_id VARCHAR,created_on TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,last_sync TEXT DEFAULT "none")');
    st.executeSql("INSERT INTO availability(coords,available_wl_1l,available_wl_35cl,available_wl_75cl,store,store_id,store_server_id,submitter) VALUES (?,?,?,?,?,?,?,?)",[coords,available_wl_1l,available_wl_35cl,available_wl_75cl,storename,itemid,store_server_id,submitter], function(st,results){
      // callback
      alertSuccess();
    });
  },function(st,err){
...
function fetchItems() {
  var q = "SELECT * FROM availability WHERE store_id = ? ORDER BY created_on DESC LIMIT 0, 10";
  ...
  db.transaction(function (t) {
    t.executeSql(q, [itemid], function (t, data) {
      ...
      for (var i =0;i<data.rows.length;i++) {
        ...
        cl += '<p>Sold in William Lawson 1L?: '+data.rows.item(i).available_wl_1l+'</p>';
        cl += '<p>Sold in William Lawson 35cl?: '+data.rows.item(i).available_wl_35cl+'</p>';
        cl += '<p>Sold in William Lawson 75cl?: '+data.rows.item(i).available_wl_75cl+'</p>';
        cl += '</a>';
      }
