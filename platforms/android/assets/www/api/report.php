...
function getStockbyAccount(){
    ...
    foreach ($outlets as $key => $outlet) {
        ...
        $query = 'SELECT store, SUM(currentstock) as currentstock, SUM(orderplaced) as orderplaced, SUM(delivered) as delivered, SUM(sale) as sale FROM data_brandstocks WHERE store_server_id = "'.$outlet->id.'" AND currentstock >= 0';
        $result = mysqli_query($db,$query) or die(mysqli_error($db));
        while ($stock = mysqli_fetch_object($result)) {
            $b = array(
                ...
                'orderplaced' => $stock->orderplaced,
                'delivered' => $stock->delivered,
                'sale' => $stock->sale
                );
            $stockdata[$outlet->account][] = $b;
        }
    }
    return $stockdata;
}
