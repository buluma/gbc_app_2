...
function saveItem() {
  var desc = document.getElementById("action_input").value;
  ...
  db.transaction(function(st) {
    ...
    st.executeSql('CREATE TABLE IF NOT EXISTS action_items (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,coords VARCHAR, description VARCHAR, submitter VARCHAR,store VARCHAR,store_id INTEGER, store_server_id VARCHAR,created_on TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,last_sync TEXT DEFAULT "none")');
    st.executeSql("INSERT INTO action_items(coords,description,store,store_id,store_server_id,submitter) VALUES (?,?,?,?,?,?)",[coords,desc,storename,itemid,store_server_id,submitter], alertSuccess);
  },function(st,err){
    console.log(err);
  },onReadyTransaction);
}
...
function fetchItems() {  
  var q = "SELECT * FROM action_items WHERE store_id = ? ORDER BY created_on DESC LIMIT 0, 10";
