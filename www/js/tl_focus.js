...
function saveItem() {
  var focus_title = document.getElementById("focus_title").value;
  ...
  db.transaction(function(st) {
    //st.executeSql('DROP TABLE IF EXISTS tl_focus_areas');
    st.executeSql('CREATE TABLE IF NOT EXISTS tl_focus_areas (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,coords VARCHAR, focus_type VARCHAR, description VARCHAR,action_input VARCHAR, start_date VARCHAR, end_date VARCHAR,submitter VARCHAR,store VARCHAR,store_id INTEGER, store_server_id VARCHAR,created_on TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,last_sync TEXT DEFAULT "none")');
    st.executeSql("INSERT INTO tl_focus_areas(coords,focus_type,description,start_date,end_date,action_input,store,store_id,store_server_id,submitter) VALUES (?,?,?,?,?,?,?,?,?,?)",[coords,focus_title,desc,focus_start_date,focus_end_date,action,storename,itemid,store_server_id,submitter], function(st,results){
      // callback
      alertSuccess();
    });
  },function(st,err){
...
function fetchItems() {
  var q = "SELECT * FROM tl_focus_areas WHERE store_id = ? ORDER BY created_on DESC LIMIT 0, 10";
  ...
  db.transaction(function (t) {
    t.executeSql(q, [itemid], function (t, data) {
      ...
      for (var i =0;i<data.rows.length;i++) {
        ...
        cl += '<p>Focus Title: '+data.rows.item(i).focus_type+'</p>';
